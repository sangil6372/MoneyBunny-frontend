...
export const useBookmarkStore = defineStore('bookmark', () => {
  ...
  const filteredBookmarks = computed(() => 
    bookmarks.value.filter(bookmark => bookmark.bookmarkId && bookmark.isBookmarked)
  );
  ...
  const fetchBookmarks = async () => {
    ...
    try {
      ...
      const transformedData = response.data.map((bookmark) => {
        ...
        supportAmount: bookmark.policyBenefitAmount ? `${bookmark.policyBenefitAmount.toLocaleString()}원` : '미정',
  ...
  const removeBookmark = async (policyId) => {
    try {
      ...
      bookmarks.value = bookmarks.value.filter(bookmark => bookmark.policyId !== policyId);
